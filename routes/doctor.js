...
router.get('/dashboard', guard, (req, res) => {
  const doctorId = getDoctorId(req.session.user.id);
  ...
  const queue = getDoctorQueue(doctorId, selectedDate);
  const stats = {
...
router.get('/queue', guard, (req, res) => {
  ...
  const queue = getDoctorQueue(doctorId, date);
...
router.post('/queue/checkin/:appointmentId', guard, (req, res) => {
  const doctorId = getDoctorId(req.session.user.id);
  const appt = db.prepare('SELECT * FROM appointments WHERE id = ? AND doctor_id = ?')
    .get(req.params.appointmentId, doctorId);
  if (!appt) return res.redirect('/doctor/queue');

  const apptDate = appt.appointment_date;
  const existing = db.prepare('SELECT id FROM queue_entries WHERE appointment_id = ? AND date = ?').get(appt.id, apptDate);
  if (!existing) {
    const maxPos = db.prepare('SELECT COALESCE(MAX(position),0) as mx FROM queue_entries WHERE doctor_id = ? AND date = ?').get(doctorId, apptDate).mx;
    db.prepare(`INSERT INTO queue_entries(patient_id, doctor_id, appointment_id, check_in_time, position, status, date) VALUES(?,?,?,datetime('now'),?,'waiting',?)`)
...
router.post('/queue/:entryId/status', guard, (req, res) => {
  const doctorId = getDoctorId(req.session.user.id);
  const { status } = req.body;
  const allowed = ['called', 'in_progress', 'completed', 'no_show'];
  if (!allowed.includes(status)) return res.redirect('/doctor/queue');

  const entry = db.prepare('SELECT * FROM queue_entries WHERE id = ? AND doctor_id = ?').get(req.params.entryId, doctorId);
  ...
  db.prepare(`UPDATE queue_entries SET ${setClause} WHERE id = ?`).run(entry.id);

  // Update related appointment status
  if (status === 'completed') {
    db.prepare(`UPDATE appointments SET status = 'completed', updated_at = datetime('now') WHERE id = ?`).run(entry.appointment_id);
  } else if (status === 'no_show') {
    db.prepare(`UPDATE appointments SET status = 'no_show', updated_at = datetime('now') WHERE id = ?`).run(entry.appointment_id);
  } else if (status === 'in_progress') {
    db.prepare(`UPDATE appointments SET status = 'in_progress', updated_at = datetime('now') WHERE id = ?`).run(entry.appointment_id);
  }

  // Reorder remaining queue
  if (['completed', 'no_show'].includes(status)) {
    reorderQueue(doctorId, entry.date);

    // Notify patient their position moved
    const nextEntry = db.prepare('SELECT * FROM queue_entries WHERE doctor_id = ? AND date = ? AND status = ? AND position = 1').get(doctorId, entry.date, 'waiting');
    if (nextEntry) {
      createNotification({
...
router.get('/schedule', guard, (req, res) => {
  const doctorId = getDoctorId(req.session.user.id);
  const schedules = db.prepare('SELECT * FROM doctor_schedules WHERE doctor_id = ? ORDER BY day_of_week').all(doctorId);
  const blockedDates = db.prepare('SELECT * FROM doctor_blocked_dates WHERE doctor_id = ? ORDER BY blocked_date').all(doctorId);
...
router.post('/schedule', guard, (req, res) => {
  const doctorId = getDoctorId(req.session.user.id);
  const { day_of_week, start_time, end_time, slot_duration, remove } = req.body;

  const day = parseInt(day_of_week);
  if (remove === '1') {
    db.prepare('DELETE FROM doctor_schedules WHERE doctor_id = ? AND day_of_week = ?').run(doctorId, day);
  } else {
    const existing = db.prepare('SELECT id FROM doctor_schedules WHERE doctor_id = ? AND day_of_week = ?').get(doctorId, day);
    if (existing) {
      db.prepare('UPDATE doctor_schedules SET start_time=?, end_time=?, slot_duration_minutes=? WHERE doctor_id=? AND day_of_week=?')
        .run(start_time, end_time, parseInt(slot_duration) || 15, doctorId, day);
    } else {
      db.prepare('INSERT INTO doctor_schedules(doctor_id, day_of_week, start_time, end_time, slot_duration_minutes) VALUES(?,?,?,?,?)')
...
// Block a date
router.post('/schedule/block', guard, (req, res) => {
  const doctorId = getDoctorId(req.session.user.id);
  const { blocked_date, reason } = req.body;
  if (!blocked_date) return res.redirect('/doctor/schedule');

  db.prepare('INSERT OR IGNORE INTO doctor_blocked_dates(doctor_id, blocked_date, reason) VALUES(?,?,?)').run(doctorId, blocked_date, reason || null);
  req.session.flash = { success: 'تم حظر اليوم.' };
  res.redirect('/doctor/schedule');
});

// Unblock a date
router.post('/schedule/unblock/:id', guard, (req, res) => {
  const doctorId = getDoctorId(req.session.user.id);
  db.prepare('DELETE FROM doctor_blocked_dates WHERE id = ? AND doctor_id = ?').run(req.params.id, doctorId);
