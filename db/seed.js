...
async function seed() {
  ...
  try {
    ...
    // Remove WAL pragma — Turso manages its own journal mode
    schema = schema.replace(/PRAGMA\s+journal_mode\s*=\s*WAL\s*;?/gi, '');
  ...
  for (const s of specialties) {
    await insertSpecialty.run(s);
  }

  // --- Admin user ---
  const adminHash = bcrypt.hashSync('Admin@12345', 12);
  await db.prepare(`INSERT INTO users(name, email, password_hash, phone, role) VALUES(?,?,?,?,?)`)
  ...
  for (const d of doctorsData) {
    const { lastInsertRowid: userId } = await insertUser.run(d.name, d.email, doctorPassword, d.phone, 'doctor');
    const spec = await getSpecialty.get(d.specialty);
    const { lastInsertRowid: docId } = await insertDoctor.run(userId, spec.id, d.avg);

    // Working days: all week (0=Sun ... 6=Sat)
    for (let day = 0; day <= 6; day++) {
      await insertSched.run(docId, day, '08:00', '16:00', d.avg);
    }
  }
  ...
  for (const p of patients) {
    await insertPatient.run(p.name, p.email, patientPassword, p.phone, 'patient');
  }
