...
(async () => {
  ...
  for (const d of newDoctors) {
    await insertUser.run(d.name, d.email, doctorPwd, d.phone, 'doctor');
    const userRow = await getUser.get(d.email);
    const userId = userRow ? userRow.id : null;
    const specRow = await getSpecialty.get(d.specialty);
    const specId = specRow ? specRow.id : null;
    if (!specId) { console.log('Specialty not found:', d.specialty); continue; }
    await insertDoctor.run(userId, specId, d.avg);
    const docRow = await getDoctor.get(userId);
    const docId = docRow ? docRow.id : null;
    if (docId) {
      for (let day = 0; day <= 4; day++) {
        await insertSched.run(docId, day, '08:00', '17:00', d.avg);
      }
    }
  ...
  for (const p of newPatients) {
    await insertUser.run(p.name, p.email, patientPwd, p.phone, 'patient');
  ...
  const allPatients = await db.prepare(`
  ...
  for (const doc of allDoctors) {
    ...
    while (appts < target && dateIdx < DATES.length) {
      ...
      const r = await insertAppt.run(pat.id, doc.id, date, slotTime, endTime, status, note || null);
      if (r.changes > 0) {
        appts++;
        totalInserted++;
      }
  ...
  const allUsers = await db.prepare(`SELECT name,email,role FROM users ORDER BY role,id`).all();
